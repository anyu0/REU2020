...
\[
  \includegraphics[scale=0.35]{graphics/MCLemma.png}
\]
\end{frame}

\begin{frame}{Strong coupling with Brownian bridges}
[Dimitrov-Wu 19] A Bernoulli random walk and a Brownian bridge with the same endpoints are ``strongly coupled'', meaning that they stay close to each other. 
\[
...
Denote $L(s)$ as a Bernoulli random walk from point $(0,0)$ to $(n,z)$, and $B(s)$ is a Brownian bridge with the same endpoints, where $s\in[0,n]$. Then we have
 $$\mathbb{P}( \sup_{s\in[0,n]} \abs*{L(s) - B(s)} \geq M \log n + x) < \epsilon$$ meaning that the maximal distance between them grows at the speed of $(\log n)^{2}$ with high probability.
\end{frame}
