...
\begin{frame}{Multiple random walks}
	...
	\end{itemize}
\[
	\includegraphics[height=0.65\textheight]{graphics/MultipleBernoulli.png}
\]
\end{frame}
...
\begin{theorem}[FFSTWZD] Let $\{\mathfrak{L}^N = (L_1^N,\dots,L_k^N)\}_{N=1}^\infty$ be a sequence of $k$ avoiding Bernoulli random walks. Fix $p\in(0,1)$ and $\lambda > 0$, and suppose that for all $n\in\mathbb{Z}$ we have
\[
...
\begin{itemize}
	\item $F_{TW}$ denotes the \textit{Tracy-Widom distribution} -- the one-point marginal for the Airy process
	
	\item {[Dauvergne-Nica-Vir\'{a}g '19]} showed that finite dimensional convergence of all curves implies tightness
	
	\item Our result shows that it suffices for the \textcolor{blue}{top curve} to converge in the f.d. sense
\end{itemize}
...
Shifting the endpoints or the bottom curve up(resp., down)\\
$\Rightarrow$ The whole line ensemble uniformly goes up(resp., down)
\[
  \includegraphics[scale=0.3]{graphics/MCLemma.png}
\]
\begin{itemize}
	\item $\mathbb{P}(L_{1}^{high}(s)>M)\geq \mathbb{P}(L_{1}^{low}(s)>M)$
\end{itemize}
\end{frame}

\begin{frame}{Strong coupling with Brownian bridges}
[Dimitrov-Wu â19] A Bernoulli random walk and a Brownian bridge with the same endpoints are ``strongly coupled'', meaning that they stay close to each other. 
\[
  \includegraphics[scale=0.4]{graphics/SCResult}
\]

Denote $L(s)$ as a Bernoulli random walk from point $(0,0)$ to $(n,z)$, and $B(s)$ is a Brownian bridge with the same endpoints, where $s\in[0,n]$. Then we have
 $$\mathbb{P}( \sup_{s\in[0,n]} \abs*{L(s) - B(s)} \geq M \log n + x) < Ke^{-\lambda x}$$ meaning that the maximal distance between them grows at the speed of $(\log n)^{2}$ with high probability.
\end{frame}
...
\begin{frame}{Controlling the minimum: pinning the bottom curve}
	...
	\end{lemma}
	\[
	\includegraphics[scale=0.10]{graphics/min2.png}
	\]
	
	\begin{itemize}
		
		\item Couple $L_k^N$ with a Brownian bridge: if ``pinned" at two points in $[r,R]$ and $[-R,-r]$, it cannot be low on scale $N^{1/3}$ on $[-r,r]$
