...
\begin{equation*}
\begin{split}
E(\vec c,\vec d, V_1^t, V_1^b)=&\left\{\tilde{\mathfrak{B}}\in \Omega_{a,S}(\vec c,\vec d)\mid (2t_2)^{1/2} V_1^t \geq \tilde{B}_1(\pm t_2) \mp p t_2 \geq \tilde{B}_{k-1}(\pm t_2) \mp p t_2 \geq (2t_2)^{1/2} V_1^b\right\}\\
A(\vec c,\vec d, V^t_1,V^b_1)=&\left\{\tilde{\mathfrak{B}}\in \Omega_{a,S}(\vec c,\vec d)\mid \tilde{B}_1\left(\pm t_1\right)\mp pt_1<\left(2t_2\right)^{1/2}V_1^t,\tilde{B}_{k-1}(\pm t_1)\pm pt_1 >\left(2t_2\right)^{1/2}V_1^b\right\}\\
...
for some $h_2$, then we have $\pr_{\tilde{\mathfrak{Q}}}(A\cap C\cap E)\geq h_1\cdot h_2=h$, then we have proved Lemma \ref{LemmaAP2}.
\end{proof}
