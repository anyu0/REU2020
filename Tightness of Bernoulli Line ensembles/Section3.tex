...
\begin{lemma}\label{BBmax}
	...
	\[
	\mathbb{P}\left(\max_{s\in[0,T]} B^\sigma_{s/T} \geq C\right) = \exp\left( - \frac{2C^2}{p(1-p)}\right) \quad \mathrm{and} \quad \mathbb{P}\left(\max_{s\in[0,T]} \big| B^\sigma_{s/T} \big| \geq C\right) = 2\sum_{n=1}^\infty \exp\left(-\frac{2n^2C^2}{p(1-p)}\right).
	\]
	In particular,
...
\begin{proof}
	...
	\end{align*}
	The second equality follows from \cite[Proposition 12.3.3]{Dudley}. The other equality follows in the same way from \cite[Proposition 12.3.4]{Dudley}.
...
\begin{lemma}\label{CurveSeparation} Fix $T\in\mathbb{N}$, $p\in (0,1)$, $C,K>0$, and $a,b\in \mathbb{Z}$ such that $\Omega(0,T,a,b)$ is nonempty. Let $\ell_{bot} \in \Omega(0,T,a,b)$. Suppose $\vec{x},\vec{y}\in\mathfrak{W}_{k-1}$, $k\geq 2$, are such that $T \geq y_i - x_i \geq 0$ for $1\leq i\leq k-1$. Write $\vec{z} = \vec{y} - \vec{x}$, and suppose that
	...
	\mathbb{P}^{0,T,\vec{x},\vec{y}}_{Ber}(E) \geq \left(1 - 3\sum_{n=1}^\infty e^{-n^2C^2/8p(1-p)}\right)^{k-1}.
	\end{equation}
	In particular, if $C \geq \sqrt{8p(1-p)\log 3}$, then we have
...
\begin{proof}
	...
	\end{split}
	\end{equation}
	By Lemma \ref{BBmax}, the first term in the second line of \eqref{SepCheb} is equal to $2\sum_{n=1}^\infty e^{-n^2C^2/8p(1-p)}$. For the second term, condition (3) in the hypothesis and Corollary \ref{Cheb} give an upper bound of $\sum_{n=1}^\infty e^{-n^2C^2/8p(1-p)}$ for sufficiently large $N$ depending on $p,C,K$, but independent of $i$. Adding these two terms and referring to \eqref{SepEst} proves \eqref{SepBound1}.
