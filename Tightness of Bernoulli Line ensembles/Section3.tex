...
\begin{lemma}\label{CurveSeparation} Fix $k, T\in\mathbb{N}$, $p\in (0,1)$. Define the constant
	\[
	...
	\end{enumerate}
	Let $\mathfrak{L} = (L_1,\dots,L_k)$ be a line ensemble with law $\mathbb{P}^{0,T,\vec{x},\vec{y}}_{Ber}$, and let $E$ denote the event that $L_1(s) \geq \cdots \geq L_k(s)$ for $s\in[0,T]$. Then we can find $W_5 = W_5(p,k)$ so that for $T\geq W_5$,
	\[
	\mathbb{P}^{0,T,\vec{x},\vec{y}}_{Ber}(E) \geq \big(1 - 3e^{-C^2/8p(1-p)}\big)^k
	\]
\end{lemma}
