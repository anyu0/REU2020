...
Fix $p,t\in(0,1)$, $k\in\mathbb{N}$, $\vec a, \vec b\in W_k$.  Suppose that $\vec{x}^{T}=(x_{1}^{T},\cdots,x_{k}^{T})$ and $\vec{y}^{T}=(y_{1}^{T},\cdots,y_{k}^{T})$ are two sequence of $k$-dimensional vectors with integer entries such that for $i\in \llbracket 1,k\rrbracket$, $$\lim_{T\rightarrow\infty}\frac{x_{i}^{T}}{\sqrt{T}}=a_{i} \quad \text{and} \quad \lim_{T\rightarrow\infty}\frac{y_{i}^{T}-pT}{\sqrt{T}}=b_{i}.$$  Let $(L_1,\dots,L_k)$ have law $\mathbb{P}^{0,T,\vec{x}^{T},\vec{y}^{T}}_{avoid,Ber}$, and define the sequence $\{Z^T\}$ of random $k$-dimensional vectors by $$Z^{T}=\left(\frac{L_{1}(tT)-ptT}{\sqrt{T}},\cdots,\frac{L_{k}(tT)-ptT}{\sqrt{T}}\right).$$ When $a_i=a_{i+1}$ or $b_{j}=b_{j+1}$ for any $i,j\in\llbracket 1,k\rrbracket$ we write
\begin{equation*}\begin{split}
\vec{a}&=(a_{1},\dots,a_{k})=(\underbrace{\alpha_{1},\dots,\alpha_{1}}_{m_{1}},\dots,\underbrace{\alpha_{p},\dots,\alpha_{p}}_{m_{p}}),\\
\vec{b}&=(b_{1},\dots,b_{k})=(\underbrace{\beta_{1},\dots,\beta_{1}}_{n_{1}},\dots,\underbrace{\beta_{q},\dots,\beta_{q}}_{n_{q}}),
\end{split}
\end{equation*}
where $\alpha_{1}>\dots>\alpha_{p}$, $\beta_{1}>\dots>\beta_{q}$ and $\sum_{i=1}^{p}m_{i}=\sum_{i=1}^{q}n_{i}=k$. Then as $T\to\infty$, $Z^{T}$ converges weakly to a random vector $\hat Z$ with density $$\rho_{\vec{a},\vec{b}}(z_{1},\dots,z_{k})=\frac{1}{Z_{\vec{a},\vec{b}}}\cdot \varphi(\vec{a},\vec{z},\vec{m})\psi(\vec{b},\vec{z},\vec{n})\prod_{i=1}^{k}e^{-c_{3}z_{i}^{2}}\mathbf{1}_{\{z_{1}>\cdots> z_{k}\}}.$$ Here, $\vec{m}=(m_{1},\dots,m_{k})$, $\vec{n}=(n_{1},\dots,n_{k})$, $c_{1},c_{2},c_{3}$ are constants depending on $p,t$, $Z_{\vec a,\vec b}$ is a normalization constant depending on $p,t,\vec{a},\vec{b}$, and $\varphi(\vec{a},\vec{z},\vec{m}), \psi(\vec{b},\vec{z},\vec{n})$ are determinants:
\begin{align*}
\varphi(\vec{a},\vec{z},\vec{m}) &= \det
\begin{bmatrix}
\left((c_{1}z_{j})^{i-1}e^{c_{1}\alpha_{1}z_{j}}\right)_{1\leq i \leq m_1, \, 1\leq j \leq k}\\
\vdots\\
\left((c_{1}z_{j})^{i-1}e^{c_{1}\alpha_{p}z_{j}}\right)_{1 \leq i \leq m_p, \, 1\leq j\leq k}
\end{bmatrix}, \\
\psi(\vec{b},\vec{z},\vec{n}) &= \det
\begin{bmatrix}
\left((c_{2}z_{j})^{i-1}e^{c_{2}\beta_{1}z_{j}}\right)_{1\leq i \leq n_1, \, 1\leq j \leq k}\\
\vdots\\
\left((c_{2}z_{j})^{i-1}e^{c_{2}\beta_{q}z_{j}}\right)_{1 \leq i \leq n_q, \, 1\leq j\leq k}
\end{bmatrix}.
\end{align*} 
\end{lemma}
Proof of this lemma will be postponed until Section \ref{Section9}, in particular in Propositions \ref{WeakConvDistinct} and \ref{WeakConvCollide}.
...
\[
\pr^{0,T,\vec x^T, \vec y^T}_{avoid,Ber}\left(\min_{1\leq i\leq k-1} \big[L_i(tT)-L_{i+1}(tT)\big]<\delta\sqrt{T}\right)<\epsilon.
\]
\end{lemma}
\begin{proof}
	We prove the claim by contradiction. Suppose there exist $M_1,M_2,\epsilon>0$ such that for any $T_0\in \mathbb{N}$ and $\delta>0$ there exists some $T\geq T_0$ with
	\[
	...
	\] 
	as $m\to\infty$ (see \cite[Theorem 3.6]{Rudin}). Denote $$Z_i^m =\frac{L_i(tT_{n_m})-ptT_{n_m}}{\sqrt{T_{n_m}}}.$$ Fix $\tilde{\delta} > 0$ and choose $M$ large enough so that if $m\geq M$ then $\delta_m < \tilde{\delta}$. Then for $m\geq M$ we have
	\begin{equation}\label{prob20inf}
	\epsilon\leq \liminf_{m\to\infty}\pr\left(\min_{1\leq i\leq k-1} \big[Z_i^m-Z_{i+1}^m\big] < \delta_{n_m}\right) \leq \liminf_{m\to\infty}\pr\left(\min_{1\leq i\leq k-1} \big[ Z_i^m-Z_{i+1}^m\big] \leq\tilde\delta\right).
	\end{equation}
	Now by Lemma \ref{prob17}, $(Z_1^m,\dots, Z_k^m)$ converges weakly to a random vector $\hat Z$. It follows from the portmanteau theorem, \cite[Theorem 3.2.11]{Durrett}, applied with the closed set $K=[0,\tilde\delta]$ that
	...
	\epsilon\leq \pr\left(\min_{1\leq i\leq k-1} \big[\hat Z_i-\hat Z_{i+1}\big]\leq\tilde\delta\right)\leq \sum_{i=1}^{k-1}\pr\left(\hat Z_i-\hat Z_{i+1}\leq \tilde\delta\right).
	\end{equation}
	This inequality holds for any $\tilde{\delta} > 0$. In order to pick a sufficient $\delta'$ to find a contradiction, we will now state the definition of $\rho$, as found in \ref{prob17}: 
