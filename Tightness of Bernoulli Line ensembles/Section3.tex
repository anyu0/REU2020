...
\begin{proof}
	Let $B^1$ be a standard Brownian bridge with variance 1 on $[0,1]$. Then $\sqrt{T}\,B^1_{s/T}$ is a standard Brownian bridge on $[0,T]$, and $\sigma B^1_{s/T}$ has the same distribution as $B^\sigma_{s/T}$. Hence
	\begin{align*}
	\mathbb{P}\Big( \max_{s\in[0,T]} B^\sigma_{s/T} \geq C \Big) &= \mathbb{P}\Big( \max_{s\in[0,T]} \sqrt{T}\,B^1_{s/T} \geq C\sqrt{T}/\sigma \Big) = e^{-2(C\sqrt{T}/\sigma)^2/T} = e^{-2C^2/p(1-p)}.
	\end{align*}
...
\begin{lemma}\label{CurveSeparation} Fix $k, T\in\mathbb{N}$, $p\in (0,1)$, and $\epsilon > 0$. Define a constant
	\[
	C = \sqrt{8p(1-p)\log\frac{3}{1-(1-\epsilon)^{1/k}}}.
	\]
	Suppose $\vec{x},\vec{y}\in\mathfrak{W}_k$ are such that for $1\leq i\leq k$,
	\begin{enumerate}[label=(\arabic*)]
		
		\item $T \geq y_i - x_i \geq 0$, 
		
		\item $x_i = x_1 - (i-1)\lceil C\sqrt{T}\rceil$ and $y_i = y_1 - (i-1)\lceil C\sqrt{T}\rceil$,
		
		\item $|y_1 - x_1 - pT| \leq K\sqrt{T}$ for a constant $K>0$.
		
	\end{enumerate}
	Let $\mathfrak{L} = (L_1,\dots,L_k)$ be a line ensemble with law $\mathbb{P}^{0,T,\vec{x},\vec{y}}_{Ber}$, and let $E$ denote the event that $L_1(s) \geq \cdots \geq L_k(s)$ for $s\in[0,T]$. Then we can find $W_5 = W_5(p,k,\epsilon)$ so that for $T\geq W_5$,
	\[
	\mathbb{P}^{0,T,\vec{x},\vec{y}}_{Ber}(E) \geq 1 - \epsilon.
	\]
\end{lemma}
