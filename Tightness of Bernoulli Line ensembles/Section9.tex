...
\begin{lemma}
	Assume the same notation as in Section \ref{SkewSchurPoly}. We then have the following asymptotic formula
\begin{align}
\begin{split}
	& h_{\lambda_{i}-x^{T}_{j}+j-i}(1^{\lfloor tT \rfloor})=\sqrt{2\pi}^{-1}\sqrt{\frac{p+1}{pt}}\sqrt{T}^{-1}\cdot\text{Exp}\bigg\{((p+1)tT)\log((p+1)tT)-(ptT)\log(ptT)\\
	& -(tT)\log(tT)+(-i+j)\log(\frac{p+1}{p})-\log(p+1)+(z_{i}-a_{j})\sqrt{T}\cdot \log(\frac{p+1}{p})-\frac{1}{2}\frac{(z_{i}-a_{j})^{2}}{p(p+1)t}+o(1)\bigg\}
\end{split}
\end{align}
where $S_{1}(p,t,T)=((p+1)tT)\log((p+1)tT)-(ptT)\log(ptT)-(tT)\log(tT)$.
\end{lemma}
\begin{proof}
...
\end{align}
We have the following Stirling's formula \cite{stirling}, 
