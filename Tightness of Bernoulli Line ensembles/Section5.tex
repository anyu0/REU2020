...
\begin{proof}(Sketch of the proof of Lemma \ref{LemmaAP2}) We introduce some suggestive notation. For $\vec{c}, \vec{d} \in \mathfrak{W}_{k-1}$ and $\epsilon > 0$ we define
	\begin{equation}
	\begin{split}
	...
	The existence of an $\epsilon$ satisfying the first inequality follows from two applications of [Problem 20]. In the first application, we take $s=s_1$ with $s_1\cdot 2t_{12}=-t_1$, and in the second application we take $s=s_2$ with $s_2\cdot 2t_{12}=t_1$. In both cases we put $M_1,M_2=\max\{\abs*{V_1^t},\abs*{V_1^b}\}$ and $\delta=\frac{9}{10}$.\footnote{Note that $s_1$ and $s_2$ actually change slightly with $N$ due to the floor functions. This can essentially be ignored for both, since we know $$\frac{r}{2r+4} \leq \frac{(r+1)N^\alpha - 1}{(2r+3)N^\alpha + 2} \leq \frac{t_1}{2t_{12}} \leq \frac{(r+1)N^\alpha + 1}{(2r+3)N^\alpha - 2} \leq \frac{r+2}{2r+2}.$$ In particular, $s_1,s_2$ lies in a compact window, which allows us to squeeze the random variable in the proof of [Cite Problem 20] and achieve the same result.}
	This yields $T_0^1, T_0^2$ and $\epsilon_1, \epsilon_2$ such that if $2t_2>\max\{T_0^1, T_0^2\}$, which can be ensured by picking a sufficiently large $N_6$ such that $N>N_6$ implies $2t_2=2\floor{(r+2)N^\alpha}>\max\{T_0^1, T_0^2\}$ and $\epsilon=\min\{\epsilon_1,\epsilon_2\}$ we obtain the first inequality in \eqref{LemmaBP2Step3}.
