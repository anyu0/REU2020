...
\begin{proof}(Sketch of the proof of Lemma \ref{LemmaAP2}) We introduce some suggestive notation. For $\vec{c}, \vec{d} \in \mathfrak{W}_{k-1}$ and $\epsilon > 0$ we define
	...
	{\bf \raggedleft Step 3.} With $V_1^t$ and $V_1^b$ as in Step 2 show that there exists an $\epsilon > 0$ sufficiently small such that for $(\vec{c}, \vec{d}) \in E$ we have
	\begin{equation}
	\frac{|B(\vec{c}, \vec{d}, \epsilon)|}{|\Omega(\vec{c}, \vec{d})|} \geq  \frac{9}{10}.
	\end{equation}
	To prove the above you simply need to invoke Problem 20.
