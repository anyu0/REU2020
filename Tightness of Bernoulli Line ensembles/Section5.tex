...
\subsection{Proof of Lemma \ref{PropSup2}}\label{Section5.2}
	...
	\begin{proof}
		...
		\noindent\textbf{Step 3.} We will now prove \eqref{21AEbound}, assuming results from Steps 3 and 4 below. We first note that by Lemma \ref{MCLxy}, if we raise the endpoints of each curve, then the probability of the event $A$ will decrease. In particular, write $T = (R-r)N^\alpha$, and define $\vec{x}\,',\vec{y}\,'$ by
		\begin{align*}
		...
		&\mathbb{P}^{rN^\alpha, RN^\alpha,\vec{x}\,',\vec{y}\,'}_{Ber} \left(\tilde{A}\cap\left\{\tilde L_1 \geq \cdots \geq \tilde L_{k-1} \geq \ell_{bot}\right\}\,\big|\,\tilde F\right) \geq \\
		&\mathbb{P}^{rN^\alpha, RN^\alpha,\vec{x}\,',\vec{y}\,'}_{Ber} \big(\tilde A\,|\,\tilde F\big) - \left( 1 - \mathbb{P}^{rN^\alpha, RN^\alpha,\vec{x}\,',\vec{y}\,'}_{Ber} \left(\tilde L_1 \geq \cdots \geq \tilde L_{k-1} \geq \ell_{bot}\,\big|\,\tilde F\right)\right). 
		\end{split}
		\end{equation}
		Here, we have written $\big(\tilde{L}_1,\dots,\tilde{L}_{k-1}\big)$ for the line ensemble with law $\mathbb{P}^{rN^\alpha, RN^\alpha,\vec{x}\,',\vec{y}\,'}_{Ber}$, and we define the events $\tilde A$, $\tilde F$ in the same way as $A$, $F$ in Step 1 but replacing $L_1^N, L_{k-1}^N$ with $\tilde{L}_i, \tilde{L}_{k-1}$. In Step 4, we will find $\tilde{N}_0$ so that for $N\geq\tilde{N}_0$, 
		\begin{align}
		& \mathbb{P}^{rN^\alpha, RN^\alpha,\vec{x}\,',\vec{y}\,'}_{Ber} \big(\tilde A\,|\,\tilde F \big) \geq 1/3, \label{21 1/3}\\
		& \mathbb{P}^{rN^\alpha, RN^\alpha,\vec{x}',\vec{y}'}_{Ber} \left(\tilde L_1 \geq \cdots \geq \tilde L_{k-1} \geq \ell_{bot}\,\big|\,\tilde F\right) \geq 11/12 \label{21 1/12},
		\end{align}
		...
		Let $\ell$ be the random variable with law $\mathbb{P}^{rN^\alpha,RN^\alpha,x_1',y_1'}_{Ber}$, and define $\tilde{A}_\ell$ in the same way as $A$ in \eqref{21AB} but with $L_1^N$ replaced by $\ell$. Since $\tilde L_1,\dots,\tilde L_k$ are independent under $\mathbb{P}^{rN^\alpha, RN^\alpha,\vec{x}\,',\vec{y}\,'}_{Ber}$, and the event $\tilde A$ in \eqref{21xyest} depends only on $\tilde L_1$, we have
		\begin{equation}\label{21indep}
		\mathbb{P}^{rN^\alpha, RN^\alpha,\vec{x}\,',\vec{y}\,'}_{Ber}\big(\tilde A\,|\,F\big) = \mathbb{P}^{rN^\alpha, RN^\alpha,x_1',y_1'}_{Ber}\big(\tilde{A}_\ell\big).
		\end{equation} 
		...
		Note that on the event $\tilde F$, $\tilde L_{k-1}$ lies uniformly below the line segment connecting $\tilde L_1(rN^\alpha)$ and $\tilde L_1(RN^\alpha)$. Thus after raising the endpoints to $\vec{x}\,',\vec{y}\,'$, both $\tilde L_{k-1}$ and $\ell_{bot}$ lie uniformly at a distance of at least $C\sqrt{T}$ below the segment $\ell_{k-2}$ connecting $\tilde L_{k-2}(rN^\alpha)$ and $\tilde L_{k-2}(RN^\alpha)$, and moreover the endpoints of all adjacent curves are at least $C\sqrt{T}$ apart. Let $\tilde{\ell}_{bot}$ denote the segment $\ell_{k-2} - C\sqrt{T}$, so that $\tilde L_{k-1} \leq \tilde\ell_{bot}$. Then since $\tilde L_1,\dots,\tilde L_{k-2},\tilde \ell_{bot}$ are independent of $\tilde F$ under $\mathbb{P}^{rN^\alpha, RN^\alpha,\vec{x},\vec{y}}_{Ber}$, we have 
		\[
		\mathbb{P}^{rN^\alpha, RN^\alpha,\vec{x},\vec{y}}_{Ber} \left(\tilde L_1 \geq \cdots \geq \tilde L_k\,\big|\,\tilde F\right) \geq \mathbb{P}^{rN^\alpha, RN^\alpha,\vec{x},\vec{y}}_{Ber} \left(\tilde L_1 \geq \cdots \geq \tilde L_{k-2} \geq \tilde \ell_{bot}\right).
		\]
