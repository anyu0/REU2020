...
\subsection{Proof of Lemma \ref{PropSup2}}
	...
	Define 
	\begin{align*}
	\overline{x} &= \lceil (M_2+1)(2t_2)^{1/2} - pt_1\rceil,\\
	\overline{y} &= \lceil(M_2+1)(2t_2)^{1/2} + pt_1\rceil.
	\end{align*}
	Fix $\delta \in (0,1)$, and define the event 
	\[
	E = \big\{\widetilde{Q}_{k-1}(-t_1) \geq \overline{x}\big\} \cap \big\{\widetilde{Q}_{k-1}(t_1) \geq \overline{y}\big\} \cap \left\{\min_{1\leq i\leq k-2} (\widetilde{Q}_i(\pm t_1) - \widetilde{Q}_{i+1}(\pm t_1)) \geq \delta(2t_2)^{1/2}\right\}.
	\]
	Since $\delta < 1$, we have $\overline{x} - \ell_{bot}(- t_1) \geq (2t_2)^{1/2} \geq \delta(2t_2)^{1/2}$ and $\overline{y} - \ell_{bot}(t_1) \geq \delta(2t_2)^{1/2}$. We split the remainder of the proof into several steps for clarity.\\
		
	\noindent\textbf{Step 1.} We first show that
		\begin{equation}\label{4.4APbound}
		E \subset \left\{Z(-t_1,t_1,\vec{x},\vec{y},\ell_{bot}) \geq \big(1-3e^{-\delta^2/8p(1-p)}\big)^{k-1}\right\}
		\end{equation}
		for large enough $N$. Here, $\vec{x} = (\widetilde{Q}_1(-t_1),\dots,\widetilde{Q}_{k-1}(-t_1))$ and $\vec{y} = (\widetilde{Q}_1(t_1),\dots,\widetilde{Q}_{k-1}(t_1))$. Define $\vec{x}\,', \vec{y}\,' \in \mathfrak{W}_{k-1}$ by
		\begin{align*}
		x_i' &= \overline{x} + (k-1-i)\lceil \delta(2t_2)^{1/2}\rceil,\\
		y_i' &= \overline{y} + (k-1-i)\lceil \delta(2t_2)^{1/2}\rceil.
		\end{align*}
		Observe that $x_i' \leq x_{k-1} + (k-1-i)\lceil \delta(2t_2)^{1/2}\rceil \leq x_i$, and similarly for $y_i$. Also, $x_{i+1}' - x_i' \leq x_{i+1} - x_i$ and $y_{i+1}' - y_i \leq y_{i+1} - y_i$ for $1\leq i\leq k-2$. By Lemma \ref{APsqueezing}, we have
		\begin{equation}\label{4.4squeeze}
		Z(-t_1,t_1,\vec{x},\vec{y},\ell_{bot}) \geq Z(-t_1,t_1,\vec{x}\,', \vec{y}\,', \ell_{bot}).
		\end{equation}
		Observe that $x_{k-1}' = \overline{x}$ and $y_{k-1}' = \overline{y}$ lie at least $\delta(2t_2)^{1/2}$ above the points $\overline{x}\,' := M_2(2t_2)^{1/2} - pt_1$ and $\overline{y}\,' := M_2(2t_2)^{1/2} + pt_1$, respectively, and moreover, $\ell_{bot}$ lies uniformly below the line segment connecting $\overline{x}\,'$ and $\overline{y}\,'$. Since we also have $\vec{x}_i' - \vec{x}_{i+1}' \geq \delta(2t_2)^{1/2}$ and $\vec{y}_i' - \vec{y}_{i+1}' \geq \delta(2t_2)^{1/2}$ for each $i$, an acceptance occurs as long as each curve $\widetilde{Q}_i$ remains within a distance $\delta(2t)^{1/2}/2$ of the line segment connecting $x_i'$ and $y_i'$. Thus, writing $\overline{z} = \overline{y} - \overline{x}$ and $T = 2t_1$, we have
		\begin{align*}
		...
		\]
		The first term is equal to $2e^{-\delta^2/8p(1-p)}$ by Lemma \ref{BBmax}, and since $|\overline{z} - pT| \leq 1$, it follows from Lemma \ref{Cheb} that the second term is $< e^{-\delta^2/8p(1-p)}$ for large enough $N$ depending on $r,p,\delta$. In view of \eqref{4.4squeeze}, we obtain \eqref{4.4APbound}.
