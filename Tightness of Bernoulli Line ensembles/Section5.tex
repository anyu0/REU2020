...
\subsection{Proof of Lemma \ref{PropSup2}}
	...
	\[
	E = \left\{\inf_{s\in[-t_1,t_1]}\big(\widetilde{Q}_{k-1}(s) - ps\big) \geq (M_2+1)(2t_2)^{1/2}\right\} \cap \left\{\min_{1\leq i\leq k-2} \big(\widetilde{Q}_i(\pm t_1) - \widetilde{Q}_{i+1}(\pm t_1)\big) \geq \delta(2t_2)^{1/2}\right\}.
	\]
	Define 
	\begin{align*}
	...
	\end{align*}
	Then on the event $E$, $\widetilde{Q}_{k-1}(-t_1)\geq \overline{x}$ and $\widetilde{Q}_{k-1}(t_1) \geq \overline{y}$. By condition (1) in the hypothesis, and since $\delta < 1$, we have in addition on $E$ that
	\begin{align*}
	...
	\noindent\textbf{Step 1.} We first show that
		...
		\end{equation}
		for large enough $N$. Define $\vec{x}\,', \vec{y}\,' \in \mathfrak{W}_{k-1}$ by
		\begin{align*}
